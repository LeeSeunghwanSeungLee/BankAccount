...
* ExceptionHandler 구현
  * 컨트롤러 이후, 비지니스 로직 수행 중에 에러를 핸들링 하기 위해 Api Exception Handler 샘플을 구현한다. [0]
  * `@/common/exception/**` 에 에러 함수가 들어가 있었음.. [0]
* 리포지토리 설계
  * 위 2가지의 엔티티(유저, 계좌)를 기반으로 h2 와 연동할 수 있는 리포지토리르 설계합니다. [0]
  * JpaRepository를 상속받아 인터페이스 설계합니다.
